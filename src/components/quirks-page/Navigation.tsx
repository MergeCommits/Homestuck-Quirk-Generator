...
import CategoryOptions from "components/quirks-page/CategoryOptions";
...
type NavigationProps = QuirkProps & {
    categories: Category[];
    ...
    onCategoryStateChange: (category: Category, state: boolean) => void;
};
...
export default function Navigation(props: NavigationProps): JSX.Element {
    ...
    const tabs = categories.map((category, index) => (
        <TabPanel key={index} index={index} activeIndex={currentTabIndex}>
            <CategoryOptions key={index} quirks={category.quirks}
                             onEnableCategoryClick={enableAll => props.onCategoryStateChange(category, enableAll)}
                             enabledQuirkViews={props.enabledQuirkViews} enabledQuirkModifiers={props.enabledQuirkModifiers}
                             onQuirkStateChange={props.onQuirkStateChange} onModifierStateChange={props.onModifierStateChange}
            />
        </TabPanel>
    ));
